...
var highlightEffect = StateEffect.define();
var clearEffect = StateEffect.define();
var semanticHighlight = StateField.define({
  provide: function(f) {
  ...
  update: function(previous, transaction) {
    const clear = transaction.effects.filter((effect) => effect.is(clearEffect)).length >= 1;
    if (clear) {
      return Decoration.none;
    } else {
      let value = Decoration.none.map(transaction.changes);
...
var Ide = class {
  constructor(parent, project, changeCallback) {
    ...
    const defaultValue = this.project.files.filter((file) => file.path).map((file) => file.contents)[0] || "";
  ...
  setValue(project) {
    ...
    this.project = project;
    const value = this.project.files.filter((file) => file.path).map((file) => file.contents)[0] || "";
    this.editor.dispatch({
