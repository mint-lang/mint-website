module References {
  const OPERATORS = {
    ...
    Mint supports a number of fixed operators. Some work on specific types some
    on all types. Whitespace before and after the operator is required except
    for `!` (negation). See the table below for specifics:
