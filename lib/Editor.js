...
const buildTheme = (fontSize, orientation) => EditorView.theme({
  "&": {
    borderBottom: orientation === "vertical" ? "3px double var(--border-color)" : "none",
    fontSize: fontSize,
    lineHeight: 1.5,
    minHeight: 0,
    minWidth: 0,
...
export default ({ parent, defaultValue, onUpdate, fontSize = "14px", orientation = "vertical" } = options) => {
  ...
  return new EditorView({
    ...
    extensions: [
      ...
      semanticHighlight,
      updateExtension,
      buildTheme(fontSize, orientation),
    ]
  });
};
